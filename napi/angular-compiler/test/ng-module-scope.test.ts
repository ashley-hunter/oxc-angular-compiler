...
describe('NgModuleScopeCollector', () => {
  ...
  describe('file processing order', () => {
    it('should resolve scope when module is collected before component file', () => {
      // This simulates the pre-scan fix: NgModule metadata is collected first
      // (during buildStart), then the component file is processed (during transform).
      // Without pre-scanning, Vite processes components before their declaring
      // module because components are dependencies of the module file.
      const collector = new NgModuleScopeCollector(createMockResolver())

      // Step 1: Pre-scan collects the NgModule file first
      collector.collectFromSource(
        `
        import { NgModule } from '@angular/core';
        import { CommonModule } from '@angular/common';
      ...
      // Step 2: Component file is processed during transform — scope should be available
      const scopeMap = collector.buildScopeMap()
      expect(scopeMap.has('MyComponent')).toBe(true)

      const scope = scopeMap.get('MyComponent')!
      const names = scope.map((d) => d.name)
      expect(names).toContain('NgForOf')
      expect(names).toContain('NgIf')
      expect(names).toContain('UpperCasePipe')
    ...
    it('should NOT resolve scope when component is processed before module', () => {
      // This demonstrates the bug: without pre-scanning, the component file
      // is transformed first and no scope is available yet.
      const collector = new NgModuleScopeCollector(createMockResolver())

      // Step 1: Component file is processed first (Vite's dependency order)
      collector.collectFromSource(
        `
      ...
      // At this point, no NgModule has been collected yet
      const scopeMap = collector.buildScopeMap()
      expect(scopeMap.has('MyComponent')).toBe(false)
    })

    it('should resolve scope after re-collection when module file is processed later', () => {
      // This shows that even after the component was processed without scope,
      // once the module is collected, the scope becomes available for subsequent calls.
      const collector = new NgModuleScopeCollector(createMockResolver())

      // Step 1: Component processed first (no scope yet)
      collector.collectFromSource(
        `
        import { Component } from '@angular/core';

        @Component({ selector: 'my-comp', template: '<p>hi</p>' })
        export class MyComp {}
        `,
        'my.comp.ts',
      )

      let scopeMap = collector.buildScopeMap()
      expect(scopeMap.has('MyComp')).toBe(false)

      // Step 2: Module processed later
      collector.collectFromSource(
        `
        import { NgModule } from '@angular/core';
        import { CommonModule } from '@angular/common';
      ...
      // Now scope is available — but the component was already transformed
      // without it! This is why pre-scanning in buildStart is needed.
      scopeMap = collector.buildScopeMap()
      expect(scopeMap.has('MyComp')).toBe(true)
      expect(scopeMap.get('MyComp')!.map((d) => d.name)).toContain('NgForOf')
    })
  })
